...
router.post('/add', (req, res) => {
    let group = req.body
    User.findById(req.body.group_creator, (err, group_creator) => {
        if (err)
            res.send({
                success: false
            })
        if (!group_creator)
            ...
            Group.addGroup(group, async(err, groupRes) => {
                if (err) {
                    console.log(err);
                    res.json({
                        success: false,
                    ...
                    let typeGroup = (Boolean(req.body.isclass)) ? 'class' : (Boolean(req.body.isEvent)) ? 'event' : 'group'
                    let emails = []
                    groupRes.members.forEach(user => {
...
router.get('/findgroupsbyname/:name', function(req, res) {
    Group.find({
        group_name: {
...
router.get('/listgroupmember/:userId', function(req, res) {
    let member = String(req.params.userId)
    console.log(member)
    Group.find({
        members: {
            $in: [member]
...
router.put('/addgroupmember/:groupId/:userId', function(req, res) {
...
router.put(':groupId/newmember/:userId', function(req, res) {
    console.log(req.params)
    Group.findOneAndUpdate({
        _id: req.params.groupId
    }, {
...
router.put('/deletemember/:groupId/:userId', function(req, res) {
    console.log(req.params)
    Group.findOneAndUpdate({
        _id: req.params.groupId
    }, {
...
router.put('/addpost/:groupId/:postId', function(req, res) {
    Group.findOneAndUpdate({
        _id: req.params.groupId
    }, {
        $push: {
            group_posts: req.params.postId
        }
    }, (err, result) => {
        if (err) {
            res.send({
...
router.put('/deletepost/:groupId/:postId', function(req, res) {
    Group.findOneAndUpdate({
        _id: req.params.groupId
    }, {
        $pull: {
...
router.get('/getgroupbyid/:id', function(req, res) {
    ...
    Group.find({
        _id: id
    }).populate('group_creator').populate('members').populate('group_posts').exec(function(err, group) {
        console.log(group)
        if (err)
            res.send({
                success: false,
...
router.get('/getchatbymembres/:userId/:userId1', function(req, res) {
    let id = req.params.userId
    let id1 = req.params.userId1
    Group.find({
        "$and": [{
                "members": {
                    "$all": [id, id1]
...
router.post('/getgroupuser', function(req, res) {
    Notice.find({
        group_creator: req.body._id
...
router.put('/update', function(req, res) {
    let updateGroup = new Group(req.body)
    Group.findOneAndUpdate({
        _id: updateGroup._id
    }, updateGroup, (err, result) => {
        if (err) {
            console.log(err)
            res.json({
                success: false
...
router.delete('/delete/:id', (req, res) => {
    let id = req.params.id
    Group.findByIdAndDelete(id, function(err) {
        if (err)
            res.send({
                success: false
            });
